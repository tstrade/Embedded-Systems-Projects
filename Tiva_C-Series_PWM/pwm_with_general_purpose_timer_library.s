...
timer_interrupt_init:
	...
	; Configure as Periodic Timer Mode
    LDR r1, [r0, #GPTMTAMR]
    ORR r1, r1, #0x002
    STR r1, [r0, #GPTMTAMR]

	; Set start value s.t. timer
	;	interrupts every 256 cycles
	MOV r1, #0x0FF
    STR r1, [r0, #GPTMTAILR]

	; Enable interrupts for Timer A-0
    LDR r1, [r0, #GPTMIMR]
    ORR r1, r1, #0x001
    STR r1, [r0, #GPTMIMR]
	...
	MOV r0, #0x0000					;
    MOVT r0, #0x4003
    LDR r1, [r0, #GPTMCTL]			; Load
	ORR r1, r1, #1					; Set first bit
	STR r1, [r0, #GPTMCTL]          ; Store back
...
; >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> START ILLUMINATE_RGB_LEDS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ;
illuminate_RGB_LED:
	PUSH {lr} 				; Spill registers to stack

	; Your code is placed here
	;r0 is either:
	;0x2 RED	0x4 BLUE	0x8 GREEN
	;0x6 PURPLE	0xA YELLOW	0xC UNDEFINED	0xE WHITE

	MOV r1, #0x5000	 				; Store base address of Port F in r1
	MOVT r1, #0x4002 				;
	MOV r2, #0x1					; Store 0001
	LDRB r3, [r1, #GPIODATA]		; Grab data byte from 0x400253FC
	AND r3, r3, r2					; Clear 1st, 2nd, and 3rd bits by ANDing with 0001
	STRB r3, [r1, #GPIODATA]			; Cleared
	ORR r3, r3, r0					; Set the flags
	STRB r3, [r1, #GPIODATA]		; Store back the ORed value

	POP {pc} 						; Restore registers from stack
; <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< END ILLUMINATE_RGB_LEDS <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ;
